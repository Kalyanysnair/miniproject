            ...
            try {
                ...
                if ($update_stmt->affected_rows > 0) {
                    ...
                    try {
                        ...
                        // Fetch user email
                        $user_stmt = $mysqli->prepare("SELECT email, username FROM tbl_user WHERE userid = ?");
                        $user_stmt->bind_param("i", $request_data['userid']);
                        $user_stmt->execute();
                        $user_result = $user_stmt->get_result()->fetch_assoc();

                        $mail->addAddress($user_result['email'], $user_result['username']);
                        $mail->Subject = "Prebooking Request Accepted";
                        $mail->Body = "Hello " . $user_result['username'] . ",\n\n" .
